...
export default function RescheduleRequestsPage() {
  ...
  const fetchRescheduleRequests = async () => {
    ...
    try {
      // For now, we'll fetch all requests. In a real app, you might want to paginate
      const response = await getApiRequest("/api/reschedule/all", token);

      if (response?.data?.success) {
        setRequests(response.data.data);
      } else {
        setError(
          response?.data?.message || "Failed to load reschedule requests"
        );
      }
    } catch (err: any) {
      setError(err.message || "Failed to load reschedule requests");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredRequests = requests
    .filter((request) => {
      const matchesSearch =
        request.attendanceId.sessionId.title
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        request.attendanceId.studentId.fullName
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-slate-600">
                Manage session reschedule requests from instructors and students
              </p>
            </div>
            <Link href="/dashboard/reschedule-requests/new">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Link href="/dashboard/reschedule-requests/instructor/all">
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-2xl p-4 hover:from-blue-100 hover:to-purple-100 transition-all duration-300 cursor-pointer">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600">View by instructor</p>
                  </div>
                </div>
              </div>
            </Link>

            <Link href="/dashboard/reschedule-requests/attendance/all">
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-4 hover:from-green-100 hover:to-emerald-100 transition-all duration-300 cursor-pointer">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600">View by session</p>
                  </div>
                </div>
              </div>
            </Link>
            ...
            <div className="bg-gradient-to-r from-orange-50 to-red-50 border border-orange-200 rounded-2xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-slate-900">Grace Period</h3>
                  <p className="text-sm text-slate-600">48-hour window</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="flex flex-col sm:flex-row gap-4 flex-1">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search by session, student, instructor, or reason..."
                  value={searchTerm}
                      ...
                      <td className="px-6 py-4">
                        <div>
                          ...
                          <p className="text-sm text-slate-600">
                            {request.attendanceId.sessionId.sessionType}
                          </p>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div>
                          ...
                          <p className="text-sm text-slate-600">
                            {request.attendanceId.studentId.email}
                          </p>
                        </div>
                      </td>
                      <td className="px-6 py-4">
